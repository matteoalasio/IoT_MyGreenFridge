...
class TemperatureThread(threading.Thread):
		...
		def run(self):
			while True:
				...
				self.MQTT_temperature.mySubscribe(topic)
				...
				if control_status != None:
					...
					self.MQTT_temperature.myPublish(topic_pub, pub_mess)
				...
				for fridge_ID in diffFridges:
					...
					for fridge in dictCurrFridges["fridges"]:
						...
						if fridge_ID == fridge["ID"]:
							...
							MQTT_temperature = Temp_MQTT(client_ID, user_ID, fridge_ID, self.broker_IP, self.broker_port, TemperatureController)
